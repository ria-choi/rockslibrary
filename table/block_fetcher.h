...
class PbaBlockFetcher {
 public:
  PbaBlockFetcher(
    uint64_t _block_size, std::string _dev_name, uint64_t _block_offset,
    FilePrefetchBuffer* _prefetch_buffer, const ReadOptions& read_options, 
    BlockContents* contents, BlockType block_type, bool _using_zstd, 
    MemoryAllocator* memory_allocator = nullptr)
    : prefetch_buffer(_prefetch_buffer),
      ...
      block_type_(block_type),
      memory_allocator_(memory_allocator){
      block_size_with_trailer = _block_size;
