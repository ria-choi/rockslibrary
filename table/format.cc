...
Status Footer::DecodeFrom(Slice input, uint64_t input_offset) {
  // std::cout << "[func call] format.cc > Footer::DecodeFrom" << std::endl;
...
std::string Footer::ToString() const {
  //std::cout << "[func call] format.cc > Footer::ToString" << std::endl;
...
Status ReadFooterFromFile(const IOOptions& opts, RandomAccessFileReader* file,
  ...
  // printf("called format.cc/ReadFooterFromFile\n");                         
  ...
  //result 출력
  std::string res = footer->ToString();
  // printf("#result#\n %s", res.c_str());
...
/* #################### 추가 ####################*/
Status ReadFooterFromFileByOffset(const int fd,
                          RandomAccessFileReader* file,
                          uint64_t file_size, Footer* footer,
                          uint64_t enforce_table_magic_number) {
  // printf("[func call] format.cc > ReadFooterFromFileByOffset\n");
  ...
  char footer_buf[Footer::kMaxEncodedLength];//std::string footer_buf;
  //AlignedBuf internal_buf;
  uint64_t read_offset = (file_size > Footer::kMaxEncodedLength)
                             ? file_size - Footer::kMaxEncodedLength
                             : 0;
  Status s;
  uint32_t footer_read_size_check;
  ...
  footer_read_size_check = read(fd, &footer_buf, Footer::kMaxEncodedLength);
  
  if(footer_read_size_check != Footer::kMaxEncodedLength){
    std::cout << "!error! size different" << std::endl;
  }
   
  //}

  Slice footer_input(footer_buf,Footer::kMaxEncodedLength);
  printf("\n----------------------------------------------------------------------------------------\n"); 
  std::cout << "#footer_read_size: " << footer_read_size_check << std::endl;
  std::cout << "#footer_input : " << std::endl;
  for(uint32_t j = 0; j < footer_read_size_check; j++) {
    printf("%d ", footer_input[j]);
  }    
  printf("\n----------------------------------------------------------------------------------------\n\n"); 
  ...
  if (!s.ok()) {
    std::cout << "footer->DecodeFrom error" << std::endl;
    return s;
  }
