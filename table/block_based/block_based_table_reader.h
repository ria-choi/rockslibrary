...
class BlockBasedTable : public TableReader {
 public:
  ...
  //Status DumpTableByOffset(const int fd);
  ...
  //추가
  struct BlockRep;
  BlockRep* get_blockrep() { return blockrep_; }
  const BlockRep* get_blockrep() const { return blockrep_; }
  
  struct BlockInfo;
  BlockInfo* get_blockinfo() { return blockinfo_; }
  const BlockInfo* get_blockinfo() const { return blockinfo_; }
  ...
  template <typename TBlockIter>
  TBlockIter* KetiNewDataBlockIterator(
 ...
 protected:
  ...
  explicit BlockBasedTable(BlockRep* _blockrep, BlockInfo* _blockinfo)
      : blockrep_(_blockrep), blockinfo_(_blockinfo), block_cache_tracer_(nullptr) {}
 ...
 private:
  ...
  template <typename TBlockIter>
  static TBlockIter* KetiInitBlockIterator(const BlockRep* blockrep, Block* block,
                                       BlockType block_type,
                                       TBlockIter* input_iter,
                                       bool block_contents_pinned);                                     
  ...
  //추가, 템플릿X
  Status RetrieveBlockByOffset(const int fd,
  ...
  Status PrefetchIndexAndFilterBlocks(
      ...
      BlockCacheLookupContext* lookup_context);
  //추가
  Status PrefetchIndexAndFilterBlocksByOffset(const int fd,
      const ReadOptions& ro, FilePrefetchBuffer* prefetch_buffer,
      InternalIterator* meta_iter, BlockBasedTable* new_table,
      bool prefetch_all, const BlockBasedTableOptions& table_options,
      const int level, size_t file_size, size_t max_file_size_for_l0_meta_pin,
  ...
  void DumpKeyValue2(const Slice& key, const Slice& value);                  
...
struct BlockBasedTable::BlockRep {
  BlockRep(const InternalKeyComparator& _internal_comparator,
      Statistics* _stats, bool _blocks_maybe_compressed,
      bool _blocks_definitely_zstd_compressed,
      const bool _immortal_table, 
      uint32_t _read_amp_bytes_per_bit, 
      BlockType _block_type, std::string _dev_name)
      : internal_comparator(_internal_comparator),
        stats(_stats) {
  ...
  SequenceNumber get_global_seqno(BlockType block_type_) const {
  return (block_type_ == BlockType::kFilter ||
          block_type_ == BlockType::kCompressionDictionary)
              ? kDisableGlobalSequenceNumber
              : global_seqno;
  }
...
struct BlockBasedTable::BlockInfo {
  BlockInfo(int _block_id, uint64_t _block_offset,uint64_t _block_size){
        block_id = _block_id;
