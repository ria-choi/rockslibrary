...
Status ReadBlockFromFileByOffset(
    const int fd,
    RandomAccessFileReader* file, FilePrefetchBuffer* prefetch_buffer,
    const Footer& footer, const ReadOptions& options, const BlockHandle& handle,
    std::unique_ptr<Block>* result, const ImmutableOptions& ioptions,
    bool do_uncompress, bool maybe_compressed, BlockType block_type,
    const UncompressionDict& uncompression_dict,
    const PersistentCacheOptions& cache_options, size_t read_amp_bytes_per_bit,
    MemoryAllocator* memory_allocator, bool for_compaction, bool using_zstd,
    const FilterPolicy* filter_policy) {
  // std::cout << "[func call] block_based_table_reader > ReadBlockFromFileByOffset" << std::endl;
  assert(result);
  ...
  BlockFetcher block_fetcher(
      ...
      do_uncompress, maybe_compressed, block_type, uncompression_dict,
      cache_options, memory_allocator, nullptr, for_compaction,
      fd /*block_fetcher에 fd 추가*/);
  Status s = block_fetcher.ReadBlockContentsByOffset();
  if (s.ok()) {
    result->reset(BlocklikeTraits<Block>::Create(
        std::move(contents), read_amp_bytes_per_bit, ioptions.stats, using_zstd,
        filter_policy));
  }
...
Status ReadBlockFromFileByPBA(
  ...
  //std::cout << "[func call] block_based_table_reader > ReadBlockFromFileByPBA" << std::endl;
  assert(result_);
  ...
  if (s.ok()) {
    result_->reset(BlocklikeTraits<Block>::Create(
        std::move(contents), read_amp_bytes_per_bit_, /*ioptions.stats*/nullptr, using_zstd,
...
void BlockBasedTable::SetupBaseCacheKey(const TableProperties* properties,
                                        ...
                                        bool* out_is_stable) {
  // std::cout << "[func call] block_based_table_reader.cc > BlockBasedTable::SetupBaseCacheKey" << std::endl;
...
Status BlockBasedTable::Open(
  ...
  if (s.ok()) {
    ...
    // new_table->DumpDataBlocks(std::cout);
...
Status BlockBasedTable::BlockOpen(
  ...
  //std::cout << "[func call] block_based_table_reader.cc > BlockBasedTable::BlockOpen "<< std::endl;
...
Status BlockBasedTable::ReadRangeDelBlock(
  ...
  // std::cout << "[func call] block_based_table_reader.cc > BlockBasedTable::ReadRangeDelBlock" << std::endl;    
...
Status BlockBasedTable::PrefetchIndexAndFilterBlocks(
    ...
    BlockCacheLookupContext* lookup_context) {
  // std::cout << "block_based_table_reader.cc > BlockBasedTable::PrefetchIndexAndFilterBlocks" << std::endl;
  Status s;
...
InternalIteratorBase<IndexValue>* BlockBasedTable::NewIndexIterator(
  ...
  assert(rep_ != nullptr); //rep_ 가 null이면 프로그램 중단
...
template <>
DataBlockIter* BlockBasedTable::KetiInitBlockIterator<DataBlockIter>(
    const BlockRep* blockrep, Block* block, BlockType block_type,
    DataBlockIter* input_iter, bool block_contents_pinned) {
  return block->KetiNewDataIterator(blockrep->internal_comparator.user_comparator(),
                                blockrep->get_global_seqno(block_type), input_iter,
                                blockrep->stats, block_contents_pinned);
}
...
Status BlockBasedTable::RetrieveBlock(
  ...
  // std::cout << "block_based_table.cc > BlockBasedTable::RetrieveBlock" << std::endl;
  assert(block_entry);
...
//template <typename TBlocklike>
Status BlockBasedTable::RetrieveBlockByOffset(const int fd,
    FilePrefetchBuffer* prefetch_buffer, const ReadOptions& ro,
    const BlockHandle& handle, const UncompressionDict& uncompression_dict,
    CachableEntry<Block>* block_entry, BlockType block_type,
    GetContext* get_context, BlockCacheLookupContext* lookup_context,
    bool for_compaction, bool use_cache, bool wait_for_cache) const {
  // std::cout << "[func call]  > BlockBasedTable::RetrieveBlockByOffset" << std::endl;
  assert(block_entry);
  assert(block_entry->IsEmpty());
...

  {
    StopWatch sw(rep_->ioptions.clock, rep_->ioptions.stats,
                 READ_BLOCK_GET_MICROS);
    s = ReadBlockFromFileByOffset(fd,
        rep_->file.get(), prefetch_buffer, rep_->footer, ro, handle, &block,
        rep_->ioptions, do_uncompress, maybe_compressed, block_type,
        uncompression_dict, rep_->persistent_cache_options,
        block_type == BlockType::kData
...
Status BlockBasedTable::RetrieveBlockByPBA(
    ...
    CachableEntry<Block>* block_entry, BlockType block_type,
    bool for_compaction, bool use_cache, bool wait_for_cache*/) const {
  //std::cout <<"[func call] block_based_table.cc > BlockBasedTable::RetrieveBlockByPBA" << std::endl;
  assert(block_entry);
  assert(block_entry->IsEmpty());
  Status s;
  // if (use_cache) {
  ...
  // const bool maybe_compressed =
  //     blockrep_->block_type != BlockType::kFilter &&
  //     blockrep_->block_type != BlockType::kCompressionDictionary &&
  //     blockrep_->blocks_maybe_compressed;
  // const bool do_uncompress = maybe_compressed;`
...
Status BlockBasedTable::DumpDataBlocks(std::ostream& out_stream) {
  // std::cout << "[func call] block_based_table_reader.cc > BlockBasedTable::DumpDataBlocks" << std::endl;
  std::unique_ptr<InternalIteratorBase<IndexValue>> blockhandles_iter(
      NewIndexIterator(ReadOptions(), /*need_upper_bound_check=*/false,
                       /*input_iter=*/nullptr, /*get_context=*/nullptr,
                       /*lookup_contex=*/nullptr));
  ...
  for (blockhandles_iter->SeekToFirst(); blockhandles_iter->Valid();
    ...
    std::cout << datablock_size << std::endl;
    datablock_size_min = std::min(datablock_size_min, datablock_size);
    datablock_size_max = std::max(datablock_size_max, datablock_size);
    datablock_size_sum += datablock_size;

    out_stream << "Data Block: # " << block_id << " | "
               << "Block Handle: " << blockhandles_iter->value().handle.ToString(true) << " | "
               << "Block Size: " << datablock_size << std::endl; 
    out_stream << "--------------------------------------\n";
