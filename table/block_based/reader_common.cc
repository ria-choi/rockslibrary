...
Status VerifyBlockChecksum(ChecksumType type, const char* data,
                           ...
                           uint64_t offset) {
  // std::cout << "[func call] reader_common.cc > VerifyBlockChecksum" << std::endl;
  ...
  // std::cout << "#checksum debug  computed: " << computed << " / stored: " <<stored << std::endl;
  ...
  if (stored == computed) {
    ...
    // std::cout << "computed: " << computed << " !=stored: " <<stored << std::endl;
    if (type == kCRC32c) {
