...
Status BlockBasedTable::IndexReaderCommon::ReadIndexBlockByOffset(
    const int fd,
    const BlockBasedTable* table, FilePrefetchBuffer* prefetch_buffer,
    const ReadOptions& read_options, bool use_cache, GetContext* get_context,
    BlockCacheLookupContext* lookup_context,
    CachableEntry<Block>* index_block) {
  // std::cout << "[func call] index_reader_common.cc > BlockBasedTable::IndexReaderCommon::ReadIndexBlockByOffset" << std::endl;
  PERF_TIMER_GUARD(read_index_block_nanos);
  ...
  const Status s = table->RetrieveBlockByOffset(fd,
      prefetch_buffer, read_options, rep->footer.index_handle(),
      UncompressionDict::GetEmptyDict(), index_block, BlockType::kIndex,
      get_context, lookup_context, /* for_compaction */ false, use_cache,
      /* wait_for_cache */ true);
...
Status BlockBasedTable::IndexReaderCommon::GetOrReadIndexBlockByOffset(
    const int fd,
    bool no_io, GetContext* get_context,
    BlockCacheLookupContext* lookup_context,
    CachableEntry<Block>* index_block) const {
  ...
  return ReadIndexBlockByOffset(fd,table_, /*prefetch_buffer=*/nullptr, read_options,
                        cache_index_blocks(), get_context, lookup_context,
                        index_block);
}
}  // namespace ROCKSDB_NAMESPACE
