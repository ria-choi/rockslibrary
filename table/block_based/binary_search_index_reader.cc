...
Status BinarySearchIndexReader::Create(
  ...
  // std::cout << "binary_search_index_reader.cc > BinarySearchIndexReader::Create" << std::endl;
...
Status BinarySearchIndexReader::KetiCreate(const int fd,
    const BlockBasedTable* table, const ReadOptions& ro,
    FilePrefetchBuffer* prefetch_buffer, bool use_cache, bool prefetch,
    bool pin, BlockCacheLookupContext* lookup_context,
    std::unique_ptr<IndexReader>* index_reader) {
  ...
  if (prefetch || !use_cache) {
    const Status s =
        ReadIndexBlockByOffset(fd,table, prefetch_buffer, ro, use_cache,
                       /*get_context=*/nullptr, lookup_context, &index_block);
    if (!s.ok()) {
      return s;
    }
...
InternalIteratorBase<IndexValue>* BinarySearchIndexReader::NewIterator(
  ...
  CachableEntry<Block> index_block;
  const Status s =
      GetOrReadIndexBlockByOffset(fd, no_io, get_context, lookup_context, &index_block);
